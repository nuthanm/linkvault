...
export default function LinkVaultApp() {
  ...
  function handleLogin(token: string, loggedInUser: User) {
    localStorage.setItem(
      AUTH_KEY,
      JSON.stringify({ token, user: loggedInUser }),
    );
    setSessionToken(token);
    ...
    <>
      ...
      >
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          {sessionToken && user && <AccountMenu user={user} />}
        </div>
      </header>
      ...
      <main
        ...
        style={{
          filter: showOverlay
            ? "blur(3px) grayscale(0.7) brightness(0.92)"
            : "none",
        ...
        <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
          <div className="flex gap-1.5">
            ...
            >
              All{" "}
              {counts.all > 0 && (
                <span className="opacity-60 text-[11px]">{counts.all}</span>
              )}
            </FilterChip>
            <FilterChip
              active={filter === "video"}
            ...
            >
              Videos{" "}
              {counts.video > 0 && (
                <span className="opacity-60 text-[11px]">{counts.video}</span>
              )}
            </FilterChip>
            <FilterChip
            ...
            >
              Articles{" "}
              {counts.article > 0 && (
                <span className="opacity-60 text-[11px]">{counts.article}</span>
              )}
            </FilterChip>
          </div>
      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-4 py-2.5 rounded-xl text-sm font-medium shadow-lg z-40 animate-slide-up whitespace-nowrap ${
